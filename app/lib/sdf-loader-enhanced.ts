...
// Metals for automatic coordination-bond detection
const DEFAULT_METALS = new Set(['FE', 'CO', 'NI', 'CU', 'ZN', 'MN', 'CR', 'V', 'TI', 'B']);
...
/**
 * Enhanced SDF loader with special handling for electron-deficient molecules like diborane
 */
export function loadSDF(text: string, options: any = {}) {
  const {
    showHydrogen = false,
    elementColors = {},
    elementRadii = {},
  ...
  // ── Enhanced bond inference for special molecules ──
  if (options.autoDetectMetalBonds !== false) {
    inferCoordinationBonds(atoms, bonds, options);
  }

  // ── Special handling for diborane and similar molecules ──
  handleSpecialMolecules(atoms, bonds, mol.properties);
  ...
  atoms.forEach((atom: any, i: number) => {
    ...
    const radius = elementRadii[symUpper as keyof typeof DEFAULT_RADII] ?? DEFAULT_RADII[symUpper as keyof typeof DEFAULT_RADII] ?? 0.3;
    const color = elementColors[symUpper as keyof typeof DEFAULT_COLORS] ?? DEFAULT_COLORS[symUpper as keyof typeof DEFAULT_COLORS] ?? 0xffffff;
  ...
  if (bonds.length) {
    ...
    const planeNormal = (() => {
      ...
      const pts = atoms.map((a: any) => new THREE.Vector3(a.x, a.y, a.z));
    ...
    bonds.forEach((bond: any) => {
      ...
      const addBond = (offsetVec: THREE.Vector3) => {
...
export function parseSDF(text: string, options: any = {}) {
  if (parseSDFInternal) {
    const result = parseSDFInternal(text);
...
/**
 * Special handling for known problematic molecules
 */
function handleSpecialMolecules(atoms: any[], bonds: any[], properties: any = {}) {
  // Check if this is diborane (B2H6)
  const formula = properties.PUBCHEM_MOLECULAR_FORMULA || '';
  const name = properties.PUBCHEM_IUPAC_NAME || '';

  console.log('=== SPECIAL MOLECULE DETECTION ===');
  ...
  if (formula === 'B2H6' || name.toLowerCase().includes('diborane')) {
    console.log('Detected diborane - adding B-B bridge bond');

    // Find the two boron atoms
    const boronAtoms = atoms
      .map((atom, index) => ({ atom, index }))
      .filter(({ atom }) => atom.symbol.toUpperCase() === 'B');

    if (boronAtoms.length === 2) {
      const [b1, b2] = boronAtoms;

      // Check if B-B bond already exists
      const existingBond = bonds.find(
        bond =>
          (bond.beginAtomIdx === b1.index + 1 && bond.endAtomIdx === b2.index + 1) ||
          (bond.beginAtomIdx === b2.index + 1 && bond.endAtomIdx === b1.index + 1)
      );

      if (!existingBond) {
        // Add B-B bridge bond
        bonds.push({
          beginAtomIdx: b1.index + 1,
          endAtomIdx: b2.index + 1,
          order: 1,
        });
        console.log('Added B-B bridge bond between atoms', b1.index + 1, 'and', b2.index + 1);
      }
    }
  }

  console.log('Final bonds:', bonds);
  console.log('===================================');
}
...
function inferCoordinationBonds(
  atoms: any[],
  bonds: any[],
  { metals = DEFAULT_METALS, cutoff = DEFAULT_CUTOFF, relFactor = DEFAULT_REL_FACTOR } = {}
) {
  const seen = new Set(bonds.map(b => `${b.beginAtomIdx}-${b.endAtomIdx}`));
  ...
  atoms.forEach((m, mi) => {
    ...
    candidates.forEach(({ li, dist }) => {
      ...
      bonds.push({ beginAtomIdx: mi + 1, endAtomIdx: li + 1, order: 0 });
      seen.add(key);
    });
  });
}
