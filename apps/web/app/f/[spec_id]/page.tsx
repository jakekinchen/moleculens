...
export default async function FigurePage({ params }: { params: { spec_id: string } }) {
  const data = await getMeta(params.spec_id);
  if (!data || data.status !== 'completed') {
    return <pre>spec_id: {params.spec_id}{"\n"}status: {data?.status ?? 'unknown'}</pre>;
  }
  const urls = data.urls || {};
  return (
    <div className="p-4">
      <h1 className="font-semibold">Figure {params.spec_id}</h1>
